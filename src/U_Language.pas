...
VAR IniFile     : TextFile;
  ...
  {Section ID des �bergebenen Sectionsname ermitteln}
  ...
  begin
    ...
    if UpperCase(Section) = UpperCase('TABText') then
      ...
      {Passende Section ID zur�ckgeben oder 0}
      if Found then
  ...
  begin
    ...
    if (PosSep<>0) and (ActSectID<>0) then begin
      ...
      {evtl. vorhandene Anf�hrungszeichen ausfiltern !}
	...
	begin
		Try
			...
			while not EOF(IniFile) do begin
				...
				{Pr�fung ob String ein Kommentar ist}
					...
					if TempSection <> 0 then begin
						...
						if ActSectID > 0 then begin
							...
							if (TempNummer <> 0) and (TempText <> '~~~') then begin
								...
								{Pr�fen ob erstes Zeichen ein "\" ist um mehrzeilige Ausgaben zu erm�glichen}
